...
/*    NSString *geocodeUrl = [NSString stringWithFormat:@"%@://maps.google.com/maps/api/geocode/xml?address=%@&sensor=false", self.useHTTP ? @"http" : @"https", [self URLEncodedString:searchQuery]];
    ...
    //use google place search api
    NSString *geocodeUrl = [NSString stringWithFormat:@"%@://maps.googleapis.com/maps/api/place/textsearch/xml?query=%@&key=%@&sensor=false", self.useHTTP ? @"http" : @"https" , [self URLEncodedString:searchQuery], APPLICATION_KEY_FOR_GOOGLE_SEARCH];
    NSLog(@"%@", geocodeUrl);
